use crate::database::models::{
    MailingList, MailingListSubscriber, NewMailingList, NewMailingListSubscriber,
};
...
) -> Result<Json<Vec<MailingListResponse>>, StatusCode> {
...
pub async fn create_mailing_list(
    pool: Extension<Arc<database::ConnectionPool>>,
    Json(data): Json<CreateMailingListRequest>,
) -> Result<Json<MailingListResponse>, ApiError> {
    use crate::database::schema::mailing_lists;
        ...
        .get_result(&mut conn)
    {
        ...
        Err(err) => {
            return Err(ApiError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                ApiErrorCode::Unknown,
                "Failed to create mailing list: ".to_string() + &err.to_string(),
                HashMap::new(),
            ))
        }
    };
    ...
    match diesel::delete(mailing_lists::table.find(mailing_list_id)).execute(&mut conn) {
        ...
        Err(err) => {
            ...
            Err(ApiError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                ApiErrorCode::Unknown,
                "Failed to delete mailing list".to_string(),
                HashMap::new(),
            ))
        }
    }
...
pub async fn subscribe_user(
    pool: Extension<Arc<database::ConnectionPool>>,
    Path(mailing_list_id): Path<i32>,
    Json(data): Json<SubscribeUserRequest>,
) -> Result<Json<SubscribeUserResponse>, ApiError> {
    ...
    let mut conn = match pool.get() {
        ...
        Err(err) => {
            return Err(ApiError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                ApiErrorCode::Unknown,
                "Failed to connect to database: ".to_string() + &err.to_string(),
                HashMap::new(),
            ))
        }
    };
        ...
        .get_result(&mut conn)
    {
        ...
        Err(err) => {
            return Err(ApiError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                ApiErrorCode::Unknown,
                "Failed to create subscriber: ".to_string() + &err.to_string(),
                HashMap::new(),
            ))
        }
    };
...
pub async fn unsubscribe_user(
    pool: Extension<Arc<database::ConnectionPool>>,
    Path(mailing_list_id): Path<i32>,
    Json(data): Json<UnsubscribeUserRequest>,
) -> Result<StatusCode, ApiError> {
    use crate::database::schema::mailing_list_subscribers;
    ...
    match diesel::delete(
        mailing_list_subscribers::table
    ...
    .execute(&mut conn)
    {
        ...
        Err(err) => {
            ...
            Err(ApiError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                ApiErrorCode::Unknown,
                "Failed to delete subscriber: ".to_string() + &err.to_string(),
                HashMap::new(),
            ))
        }
    }
...
pub async fn send_mailing_list_mails(
    pool: Extension<Arc<database::ConnectionPool>>,
    Path(mailing_list_id): Path<i32>,
    Json(data): Json<SendMailsRequest>,
) -> Result<StatusCode, ApiError> {
    use crate::database::schema::mailing_list_subscribers;
        ...
        .load(&mut conn)
    {
        ...
        Err(err) => {
            return Err(ApiError::new(
                StatusCode::INTERNAL_SERVER_ERROR,
                ApiErrorCode::Unknown,
                "Failed to load subscribers: ".to_string() + &err.to_string(),
                HashMap::new(),
            ))
        }
    };
    ...
    for subscriber in subscribers {
        ...
        placeholder_data.insert(
            ...
            Value::String(subscriber.name),
        );
        placeholder_data.insert(
            "subscriber_email".to_string(),
            Value::String(subscriber.email.clone()),
        );
